...
import { View, StyleSheet , Platform , ActivityIndicator , Keyboard, Image} from "react-native";
...
_submitForm = () => {
  ...
  if (_.isEmpty(fullName)) {
    Util.showAlert("Error", "Full Name is Required");
    setTimeout(() => {
      this.name.focus();
    }, 500);
  } 
  else if (_.isEmpty(email)) {
    Util.showAlert("Error", "Email is Required");
    setTimeout(() => {
      this.email.focus();
    }, 500);
  } 
  else if (!Util.isEmailValid(email)) {
    Util.showAlert("Error", "Email is Invalid");
    setTimeout(() => {
      this.email.focus();
    }, 500);
  } else if (_.isEmpty(password)) {
    Util.showAlert("Error", "Password is Required");
    setTimeout(() => {
      this.passwordRef[0].focus();
    }, 500);
  } else if (_.isEmpty(confirmPassword)) {
    Util.showAlert("Error", "Confirm password is Required");
    setTimeout(() => {
      this.passwordRef[1].focus();
    }, 500);
  } else if (confirmPassword !== password) {
    Util.showAlert("Error", "Password & Confirm Password Mismatch");
    setTimeout(() => {
      this.passwordRef[1].focus();
    }, 500);
  } else {
...
_content = () => {
  if (
    ...
    <View style={styles.buttonView}>      
      <ButtonView style={[styles.button,styles.button2]} onPress={this._submitForm}>
        <Text>Sign Up</Text>
...
_onAccessoryPress = () => {
  ...
  render() {
    ...
    const { email,  password, confirmPassword ,errors, secureTextEntry } = this.state;
    ...
    return (
      ...
      <Text size="xxLarge" color="primary" type="black">SIGN UP</Text>
